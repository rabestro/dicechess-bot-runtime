 ...
 * }
 *
 * <p>Every public type here speaks only {@code String}, {@code java.util.List}, and {@code
 * java.util.Map} — no Gson type, and nothing library-specific, crosses the public boundary. A
 * bot written in Kotlin or Scala calls this exactly like a Java bot would; the strategy itself is
 * a plain {@code java.util.function.Function}, so a lambda, a method reference, or a Scala
 * function converted via {@code asJava} all work unchanged.
 *
 * <h2>What is deliberately not here</h2>
 *
 * <p>Game logic, DFEN parsing, and move legality are the bot author's concern (or the real
 * engine's, if the bot links one) — this package only delivers a position string to the strategy
 * function and relays back whatever moves it returns. It also does not read or write an opening
 * book itself; {@link lv.id.jc.dicechess.runtime.JsonFiles} is a generic string-map loader a
 * strategy can use for that, or for any similarly simple lookup table.
 */
